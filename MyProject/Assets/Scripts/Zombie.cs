...
public class Zombie : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (playerController.GetNumberOfZombiesKilledGlobal() >= 10)
        {
            animator.SetFloat("speed", 4.0f);
        }

        if (playerController.GetNumberOfZombiesKilledTemp()%10 == 0 && playerController.GetNumberOfZombiesKilledTemp() != 0)
        {
            playerController.SetNumberOfZombiesKilledTemp(0);
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.tag == "Bullet")
        {
            ...
            if (currentHealth <= 0)
            {
                ...
                playerController.IncreaseNumberOfZombiesKilledTemp();
                playerController.IncreaseNumberOfZombiesKilledGlobal();
            }
        }
